...
for pkt in packets:
    if pkt.haslayer(Ether):
        ...
        # elif pkt[Ether].type == ETH_ARP:
        #     #ARP
        #     if pkt.haslayer(TCP):
        #         data.append([time.time(), pkt[Ether].dst, pkt[Ether].src, pkt[Ether].type, pkt[ARP].ttl, pkt[ARP].proto, pkt[ARP]].src, pkt[IP].dst, pkt[TCP].sport, pkt[TCP].dport])
        #     elif pkt.haslayer(UDP):
        #         data.append([time.time(), pkt[Ether].dst, pkt[Ether].src, pkt[Ether].type, pkt[IP].ttl, pkt[IP].proto, pkt[IP].src, pkt[IP].dst, pkt[UDP].sport, pkt[UDP].dport])
        #     else:
        #         protocol = "Other"
        #         sport = None
        #         dport = None
        #         data.append([time.time(), pkt[Ether].dst, pkt[Ether].src, pkt[Ether].type, pkt[IP].ttl, pkt[IP].proto, pkt[IP].src, pkt[IP].dst, sport, dport])


        elif pkt[Ether].type == ETH_IPV6:
            if pkt.haslayer(TCP):
                data.append([time.time(), pkt[Ether].dst, pkt[Ether].src, pkt[Ether].type, pkt[IP].ttl, pkt[IP].proto, pkt[IP].src, pkt[IP].dst, pkt[TCP].sport, pkt[TCP].dport, len(pkt)])
            elif pkt.haslayer(UDP):
                data.append([time.time(), pkt[Ether].dst, pkt[Ether].src, pkt[Ether].type, pkt[IP].ttl, pkt[IP].proto, pkt[IP].src, pkt[IP].dst, pkt[UDP].sport, pkt[UDP].dport, len(pkt)])
...
with open(r"C:\Users\bubbl\Desktop\보안탐구\연습장\유튜브\mainpc_normal_3_3.csv", "w", newline="") as file:
